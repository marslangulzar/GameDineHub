...
ilms.routers = {
    // Controller Routes
    UpdateAppSetting: "/Account/UpdateAppSetting",
    UpsertUser: "/Account/UpsertUser",
    ChangePassword: "/Account/ChangePassword",
    ActiveInActiveUser: "/Account/ActiveInActiveUser",
    SearchSongs: "/Team/SearchSongs",
    GetVotingSong: "/Team/GetVotingSong",
    RegisterTeam: "/Team/RegisterTeam",
    TheamCast: "/General/ThemeCast",
    Voting: "/General/Voting",
    /*GetTeamName: "/Team/GetTeamName"*/
};
...
ilms.ajaxPostCallAsync = function (pageUrl, dataObj, callback, btnForLoading, showOverlay = true) {
    if (!ilms.isNullOrEmpty(btnForLoading)) {
        var btnOriginalHtml = $(btnForLoading)[0].innerHTML;
        ilms.showLoadingToButtonAndReturnOriginalHtml(btnForLoading);
    }
...
ilms.partialPageLoadingPost = function (pageUrl, dataObj) {
    ...
    $.ajax({
        ...
        success: function (data, status, xhr) {
            ...
            ilms.hideLoading();
            //=== Temporary Working Will Remove After
            if (stepID === 200) {
                $('#divSearch').show();
            }
    ...
    Swal.fire(
        {
            ...
            text: message,
            type: ilms.isNullOrEmpty(type) ? "warning" : type,
...
ilms.markMatch = function (text, term) {
