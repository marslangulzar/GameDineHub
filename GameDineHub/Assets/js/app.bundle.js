...
var initApp = (function (app) {
    ...
    /**
	 * Load Styles 
	 * usage: initApp.loadScript("css/my_lovely_style.css");
	 */
    app.loadStyle = function (styleName) {
        if (!myapp_config.cssArray[styleName]) {
            var promise = jQuery.Deferred();
            //<link href="/Assets/css/vendors.bundle.css" rel="stylesheet">
            /* adding the stlye tag to the head as suggested before */
            ...
            myapp_config.cssArray[styleName] = promise.promise();
        }
        else if (myapp_config.debugState)
            console.log("This Style was already loaded: " + styleName);
    };
	/**
    ...
    app.domReadyMisc = function () {
        ...
        if (typeof $.fn.slimScroll !== 'undefined' && myapp_config.thisDevice === 'desktop') {
            ...
            $('.custom-scroll:not(.disable-slimscroll) >:first-child').slimscroll({
                height: $(this).data('scrollHeight') || '90%',
        ...
        myapp_config.root_
            .on('click touchend', '[data-action]', function (e) {
                ...
                switch (true) {
					 ...
					 * load onDemand Style
					 **/
                    case (actiontype === 'app-loadstyle'):

                        var loadurl = $(this).attr('data-loadurl');

                        initApp.loadStyle(loadurl);
...
/* =========================== CUSTOM SCRIPT BY NAVEED UL ISLAM  chaudhrynavid@gmail.com */
/*
 * LOAD SCRIPTS
 * Usage:
...
function drawBreadCrumb(opt_breadCrumbs) {
    ...
    bread_crumb.empty(),
        bread_crumb.append($("<li>Home</li>")), a.each(function () {
            bread_crumb.append("&nbsp;&nbsp;/&nbsp;&nbsp;").append($("<li></li>").html($.trim($(this).clone().children(".badge").remove().end().text()))), --b || (document.title = bread_crumb.find("li:last-child").text())
        });
...
function pageSetUp() {
    if (myapp_config.debugState)
        console.log("%c✔ Device Detected as: " + myapp_config.thisDevice, "color: #cc2878");
    ...
    if (myapp_config.thisDevice === "desktop") {
        ...
        $("[rel=popover-hover], [data-rel=popover-hover], [data-toggle=popover]").popover({
            trigger: "hover"
        });
...
/* Stop watch 
 USAGE: <div class="fs-xl"><i class="fa fa-clock-o flip animated"></i> <span id="hour">00</span>:<span id="minute">00</span>:<span id="second">00</span></div>
...
function startTimer(oldCounter) {
    //debugger
    oldCounterArr = oldCounter != undefined && oldCounter != '' ? oldCounter.split(':') : [];
    if (myapp_config.debugState) {
        console.log(oldCounter);
        console.log(oldCounterArr);
    }
    if (oldCounterArr === undefined || oldCounterArr.length > 0) {
        seconds = oldCounterArr[2] == '00' ? '0' : oldCounterArr[2].startsWith('0') ? oldCounterArr[2].substr(1, 1) : oldCounterArr[2];
        minutes = oldCounterArr[1] == '00' ? '0' : oldCounterArr[1].startsWith('0') ? oldCounterArr[1].substr(1, 1) : oldCounterArr[1];
        hours = oldCounterArr[0] == '00' ? '0' : oldCounterArr[0].startsWith('0') ? oldCounterArr[0].substr(1, 1) : oldCounterArr[0];
    }
    clearTimeout(timer);
